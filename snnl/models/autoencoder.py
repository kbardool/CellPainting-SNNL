...
from snnl.utils import display_epoch_metrics, take_checkpoint
...
class Autoencoder(Model):
    ...
    def setup_prim_optimizer(self):
        print(f"setup_prim_optimizer()")
        params = self.network_params
        if self.use_single_loss and not self.use_annealing:
    ...
    def fit(self, args, data_ldrs, DEBUG_COUNT = 0 ):
        header = True
        train_loader = data_ldrs['train']
        val_loader = data_ldrs['val']
        for epoch in range(self.starting_epoch, self.ending_epoch):

            train_loss = self.epoch_train(train_loader, epoch, DEBUG_COUNT = DEBUG_COUNT)
            val_loss = self.epoch_validate(val_loader, epoch)

            display_epoch_metrics(self, epoch, self.ending_epoch, header)
            header = False
            self.scheduling_step(val_loss)

            if args.WANDB_ACTIVE:
                epoch_metrics = {x : y for x,y in self.training_history['gen'].items()} | \
                                {x : y[-1] for x,y in self.training_history['val'].items()} | \
                                {x : y[-1] for x,y in self.training_history['trn'].items()} 
                wandb_log_metrics(data = epoch_metrics, step = epoch)

            if self.new_val_best :
                take_checkpoint(self, args, epoch+1, update_best = True)

            if (epoch + 1) % args.save_every == 0:
                take_checkpoint(self, args, epoch+1)
 
        print(f"Last Epoch {epoch+1}")
        take_checkpoint(self, args, epoch+1, update_latest = True)
